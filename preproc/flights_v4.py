...
def preprocess(data, number_of_airports_included = 15 , number_of_airlines_included = 10, airline_boolean = 0 , taxi_in_out_boolean = 0, define_cut_off = 0.75, sample_size = 20000):
    ...
    def select_number_of_airports(data, number_of_airports_included):
        ...
        main_origins = data[["Origin", "Dest"]].groupby(["Origin"]).count().sort_values(by="Dest", ascending=False).head(number_of_airports_included).reset_index()["Origin"]

        main_dests = data[["Dest", "Origin"]].groupby(["Dest"]).count().sort_values(by="Origin", ascending=False).head(number_of_airports_included).reset_index()["Dest"]
    ...
    def dummy_generator(data, taxi_in_out_boolean, airline_boolean):
        ...
        dum_dest = dum_dest.drop(['ATL'], axis=1)
        dum_air = pd.get_dummies(data["IATA_CODE_Reporting_Airline"])
        dum_air = dum_air.drop(['WN'], axis=1)
    ...
    def dummy_day(data):
        ...
        week.columns = ["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"]
    ...
    data.drop(["DayOfWeek","DepDel15","CRSDepTimeHour","CRSDepTime",  "CRSArrTime", 
